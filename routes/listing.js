...
router.get('/:id/booking', isLoggedIn, wrapAsync(async (req, res) => {
    ...
    res.render('listings/booking.ejs', { listing: listing, messages: req.flash() });
}));

router.post("/:id/booking", isLoggedIn, wrapAsync(async (req, res) => {
    let { id } = req.params;
    const { customername, email, address } = req.body;

    if (  !email || !address) {
        req.flash("success", "Booking successful. Welcome to my Hotel .");
        ...
        try {
            ...
            req.flash("error", "Booking not successful!");
            return res.redirect(`/listings/${id}/booking/success`);
        } catch (error) {
...
router.get("/:id/booking/success", isLoggedIn, wrapAsync(async (req, res) => {
    let { id } = req.params;
    const listing = await Listing.findById(id);
    res.render("bookingSuccess.ejs", { listing: listing });
}));
